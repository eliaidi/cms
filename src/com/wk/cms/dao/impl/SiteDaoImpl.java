...
public class SiteDaoImpl implements ISiteDao {
	...
	public List<Site> findAll() {
		...
		return (List<Site>) hibernateTemplate.find("select s from Site s order by s.crTime asc");
	}
	@Override
	...
	public Site findByName(String name) {
		...
		List<Site> sites = (List<Site>) hibernateTemplate.find("select s from Site s where s.name=?", name);
...
//		hibernateTemplate.bulkUpdate("delete from Site s where s.id=?", siteId);
	}
